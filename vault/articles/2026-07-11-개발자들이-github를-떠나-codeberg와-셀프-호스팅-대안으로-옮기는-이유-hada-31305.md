---
category: AI
collected_at: '2026-07-11T05:34:11+09:00'
geeknews_comments: 1
geeknews_score: 1
geeknews_url: https://news.hada.io/topic?id=31305
id: hada-31305
matched_keywords:
- AI
read: false
recommend_score: 2.901
recommended_on: '2026-07-11'
source: geeknews
tags:
- AI
- Other
- howtogeek.com
title: 개발자들이 GitHub를 떠나 Codeberg와 셀프 호스팅 대안으로 옮기는 이유
url: https://www.howtogeek.com/why-developers-are-ditching-github-for-codeberg-and-self-hosting-alternatives/
---

## TL;DR
- 이 글은 개발자들이 GitHub 대신 Codeberg와 셀프 호스팅 대안으로 옮기는 이유를 다룬다.
- GitHub의 서비스 장애와 정치적 논란은 많은 오픈소스 프로젝트가 대체 플랫폼으로 이동하는 원인으로 작용하고 있다.
- 독자들은 이러한 변화가 소프트웨어 개발 생태계에 미치는 영향과 다양한 대안 서비스를 이해하는 데 도움이 될 것이다.

## GeekNews 요약
- GitHub는 2025년에 **6억 개 이상 저장소**와 거의 10억 건의 커밋을 기록한 대형 플랫폼이지만, 일부 오픈소스 프로젝트는 장애와 운영 방향에 대한 불신 때문에 다른 호스팅을 선택하고 있음
- Ghostty, Zig, Tenacity는 GitHub를 떠났거나 **읽기 전용 미러**만 남겼고, Dillo와 Hare도 이전했으며 GNOME과 Apache처럼 처음부터 셀프 호스팅을 택한 프로젝트도 있음
- 가장 직접적인 불만은 **서비스 장애**로, IncidentHub는 2025년 5월부터 1년 동안 GitHub의 “major outages” 48건과 총 112시간 다운타임을 추적함
- 정치적 논란으로는 GitHub의 ICE 관련 **20만 달러 계약**이 언급됐고, AI 통합 확대와 2025년 CEO Thomas Dohmke의 “Either you embrace AI, or get out of this career.” 발언도 반발을 키움
- Codeberg, GitLab, Bitbucket, Sourcehut, Gitea, Forgejo는 이슈 추적, 정적 페이지 호스팅, CI/CD, 이메일 기반 워크플로, 자체 포지 운영 등 GitHub 밖의 현실적인 선택지를 제공함

---

## 원문
- [원문](https://www.howtogeek.com/why-developers-are-ditching-github-for-codeberg-and-self-hosting-alternatives/)
- [GeekNews 토론](https://news.hada.io/topic?id=31305)

## My Note
<!-- 한 줄 코멘트 남기기 -->
