---
category: AI
collected_at: '2026-07-29T11:33:24+09:00'
geeknews_comments: 1
geeknews_score: 1
geeknews_url: https://news.hada.io/topic?id=31936
id: hada-31936
matched_keywords:
- AI
- LLM
read: false
recommend_score: 4.901
recommended_on: '2026-07-30'
source: geeknews
tags:
- AI
- Other
- arxiv.org
title: 'Kimi Linear: 표현력과 효율을 높인 어텐션 아키텍처(2025)'
url: https://arxiv.org/abs/2510.26692
---

## TL;DR
- 이 글은 Kimi Linear라는 새로운 어텐션 아키텍처의 구조와 성능에 대해 다룬다.
- Kimi Linear는 KDA와 MLA를 결합하여 학습 효율성을 100% 향상시키고, 출력 속도는 MLA보다 6.3배 빨라졌다.
- 이는 다양한 NLP 작업에서 더 빠르고 효율적인 모델 개발을 가능하게 하여 연구자와 실무자에게 중요한 시사점을 제공한다.

## GeekNews 요약
- **Kimi Linear**는 KDA와 MLA를 3:1로 배치한 하이브리드 구조로, 동일한 학습 조건에서 전체 MLA보다 단기·장기 문맥과 강화학습 평가 전반에서 높은 성능을 기록함
- 핵심 모듈인 **Kimi Delta Attention(KDA)** 은 Gated DeltaNet의 헤드 단위 망각 게이트를 채널 단위로 세분화해, 유한한 RNN 메모리의 각 특성 차원이 독립적인 망각률을 갖도록 함
- 특수한 **Diagonal-Plus-Low-Rank(DPLR)** 전이 행렬과 청크 단위 병렬 알고리듬으로 일반 DPLR보다 연산량을 줄였으며, 연산자 효율은 약 100% 향상됨
- 활성 파라미터 3B·전체 파라미터 48B 모델을 1.4T 토큰으로 학습해 MMLU-Pro 4K에서 51.0점, RULER 128K에서 84.3점과 3.98배 가속을 달성했으며, 1M 토큰에서는 MLA보다 **출력 토큰당 시간이 6.3배 빨라짐**
- 장문 생성 시 **KV 캐시를 최대 75% 절감**하면서 기존 전체 어텐션 파이프라인의 캐시·스케줄링 인터페이스를 바꾸지 않고 적용할 수 있고, KDA 커널과 vLLM 구현 및 모델 체크포인트도 공개됨

---

## 원문
- [원문](https://arxiv.org/abs/2510.26692)
- [GeekNews 토론](https://news.hada.io/topic?id=31936)

## My Note
<!-- 한 줄 코멘트 남기기 -->
