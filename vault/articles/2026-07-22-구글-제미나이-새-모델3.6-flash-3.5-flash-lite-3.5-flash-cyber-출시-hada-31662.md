---
category: AI
collected_at: '2026-07-22T02:03:32+09:00'
geeknews_comments: 0
geeknews_score: 2
geeknews_url: https://news.hada.io/topic?id=31662
id: hada-31662
matched_keywords:
- AI
read: false
recommend_score: 3.099
recommended_on: '2026-07-27'
source: geeknews
tags:
- AI
- Other
- blog.google
title: 구글 제미나이 새 모델(3.6 Flash, 3.5 Flash-Lite, 3.5 Flash Cyber) 출시
url: https://blog.google/innovation-and-ai/models-and-research/gemini-models/gemini-3-6-flash-3-5-flash-lite-3-5-flash-cyber/
---

## TL;DR
- 구글은 제미나이 시리즈의 새로운 모델인 3.6 Flash, 3.5 Flash-Lite, 3.5 Flash Cyber를 출시했다.
- 3.6 Flash는 이전 모델보다 향상된 코딩 및 멀티모달 성능을 제공하며 비용과 출력 토큰 사용량을 줄인다.
- 이러한 기술 발전은 AI 에이전트 개발의 효율성을 높이고 다양한 산업에서의 활용 가능성을 강화한다.

## GeekNews 요약
• 구글은 AI 에이전트 개발의 효율성, 낮은 지연 시간, 신뢰성을 위해 설계된 새로운 제미나이 모델인 3.6 Flash, 3.5 Flash-Lite, 3.5 Flash Cyber를 출시

• Gemini 3.6 Flash는 3.5 Flash에 비해 향상된 코딩, 지식 작업, 멀티모달 성능을 제공하며, 출력 토큰 사용량을 줄이고 비용을 절감

• Gemini 3.5 Flash-Lite는 3.5 클래스에서 가장 빠르고 비용 효율적인 모델로, 에이전트 검색 및 문서 처리와 같은 고처리량 작업에 적합

• Gemini 3.5 Flash Cyber는 사이버 보안 취약점에 맞게 조정된 특수 모델로, CodeMender를 통해 제한된 접근 시범 프로그램을 통해 제공

• Gemini 3.5 Pro는 곧 더 넓은 범위로 출시될 예정이고 Gemini 4의 개발이 진행 중

## 원문
- [원문](https://blog.google/innovation-and-ai/models-and-research/gemini-models/gemini-3-6-flash-3-5-flash-lite-3-5-flash-cyber/)
- [GeekNews 토론](https://news.hada.io/topic?id=31662)

## My Note
<!-- 한 줄 코멘트 남기기 -->
