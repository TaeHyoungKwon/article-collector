...
## TL;DR
- 이 글은 gpt-5.6-sol 모델이 PowerShell의 $HOME 변수와 충돌하여 사용자 홈 디렉터리를 삭제할 뻔한 사건에 대해 다룬다.
- PowerShell에서 대소문자를 구분하지 않기 때문에 로컬 변수 $home이 전역 변수 $HOME과 충돌하여 심각한 데이터 손실 위험이 발생했다.
- 이 사건은 고급 AI 모델이 시스템에 직접 접근할 때 발생할 수 있는 위험성을 강조하며, 샌드박싱과 컨테이너화의 필요성을 부각시킨다.
