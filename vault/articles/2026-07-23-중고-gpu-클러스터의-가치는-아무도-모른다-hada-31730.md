---
category: AI
collected_at: '2026-07-23T20:20:18+09:00'
geeknews_comments: 1
geeknews_score: 2
geeknews_url: https://news.hada.io/topic?id=31730
id: hada-31730
matched_keywords:
- AI
read: false
recommend_score: 3.307
recommended_on: '2026-07-24'
source: geeknews
tags:
- AI
- Other
- substack.com/ciphertalk
title: 중고 GPU 클러스터의 가치는 아무도 모른다
url: https://ciphertalk.substack.com/p/nobody-knows-what-a-used-gpu-cluster
---

## TL;DR
- 중고 GPU 클러스터의 가치는 운영 상태와 팀의 경험에 따라 크게 달라진다.
- GPU 담보 금융이 확산되고 있으나, 가격 산정 인프라 부족으로 리스크 측정이 어려워 대출 금리가 높아진다.
- 이러한 상황은 투자자와 기업이 중고 GPU 자산의 가치를 평가하는 데 신중해야 한다는 점을 시사한다.

## GeekNews 요약
- xAI가 채무불이행하면 대주단은 20만 개 GPU로 구성된 Colossus를 인수해 임대할 수 있지만, 실제 가치는 장비보다 **운영 상태와 팀의 암묵지**에 크게 좌우됨
- xAI와 CoreWeave를 중심으로 **GPU 담보 금융**이 확산됐으나 가격 산정 인프라가 부족해, 기준금리보다 약 8.5%포인트 높은 대출 금리 등에 측정하기 어려운 위험이 반영됨
- H100 임대료는 2024년 초 시간당 약 8달러에서 2025년 10월 1.70달러로 하락했다가 2026년 3월 2.35달러로 반등해 **잔존가치와 미래 임대수익**을 예측하기 어려움
- 동일한 GPU의 내용연수도 CoreWeave는 6년, Nebius는 4년으로 잡으며, NVIDIA의 제품 주기가 2년에서 1년으로 단축돼 **장부가치와 회수가치**의 격차가 커질 수 있음
- GPU 담보에는 장부상 액면가, 긴급 매각가, 정상 운영가라는 서로 다른 가치가 존재하며, **운영 인계에 실패하면** 계약상 인수한 클러스터를 수익 자산으로 유지하기 어려움

---

## 원문
- [원문](https://ciphertalk.substack.com/p/nobody-knows-what-a-used-gpu-cluster)
- [GeekNews 토론](https://news.hada.io/topic?id=31730)

## My Note
<!-- 한 줄 코멘트 남기기 -->
