---
category: AI
collected_at: '2026-05-15T12:34:33+09:00'
geeknews_comments: 1
geeknews_score: 1
geeknews_url: https://news.hada.io/topic?id=29535
id: hada-29535
matched_keywords:
- AI
- LLM
read: false
recommend_score: -995.099
recommended_on: '2026-05-15'
source: geeknews
tags:
- AI
- Other
- twitter.com/tdietterich
title: '새 arXiv 정책: 환각 참고문헌에 1년 이용 금지'
url: https://twitter.com/tdietterich/status/2055000956144935055
---

## TL;DR
- 이 글은 arXiv의 새로운 정책에 따라 환각 참고문헌이 포함된 논문에 대한 1년 이용 금지 조치를 다룬다.
- 저자는 생성형 AI의 결과를 검증하지 않을 경우 논문 전체에 대한 책임을 져야 하며, 이것이 적용되는 기준이 명확히 제시된다.
- 이 정책은 연구의 신뢰성을 높이고, 연구자들이 AI 사용에 신중해지도록 유도하는 데 의미가 있다.

## GeekNews 요약
- **arXiv Code of Conduct**는 저자로 이름을 올린 사람이 생성 방식과 무관하게 논문 전체에 책임진다고 규정함
- 생성형 AI가 만든 **부적절한 언어**, 표절, 편향, 오류, 잘못된 참고문헌이 논문에 들어가도 책임은 저자에게 있음
- arXiv는 저자가 LLM 생성 결과를 검증하지 않았다는 **명백한 증거**가 있으면 논문 전체를 신뢰할 수 없다고 봄
- 제재는 **arXiv 1년 이용 금지**이며, 이후 제출물은 먼저 평판 있는 동료심사 학술 venue에서 수락돼야 함
- **환각 참고문헌**이나 “실험의 실제 숫자로 채우라” 같은 LLM 메타 코멘트가 명백한 증거로 간주됨

---

## 원문
- [원문](https://twitter.com/tdietterich/status/2055000956144935055)
- [GeekNews 토론](https://news.hada.io/topic?id=29535)

## My Note
<!-- 한 줄 코멘트 남기기 -->
