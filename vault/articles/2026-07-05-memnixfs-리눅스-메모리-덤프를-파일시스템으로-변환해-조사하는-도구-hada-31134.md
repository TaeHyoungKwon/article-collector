...
## TL;DR
- MemNixFS는 리눅스 메모리 덤프를 파일 시스템 형태로 변환하여 조사하는 포렌식 도구이다.
- AVML, LiME, raw 및 kdump 이미지를 지원하며, 기존 포렌식 도구가 그대로 활용 가능하다는 특징이 있다.
- 이 도구는 메모리 포렌식 작업을 일반 파일 조작과 유사하게 수행할 수 있어 많은 분석가에게 유용하다.
