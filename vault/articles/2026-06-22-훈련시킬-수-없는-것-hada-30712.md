---
category: AI
collected_at: '2026-06-22T11:07:02+09:00'
geeknews_comments: 0
geeknews_score: 7
geeknews_url: https://news.hada.io/topic?id=30712
id: hada-30712
matched_keywords:
- AI
read: false
recommend_score: -995.921
recommended_on: '2026-06-23'
source: geeknews
tags:
- AI
- Other
- saranormous.substack.com
title: 훈련시킬 수 없는 것
url: https://saranormous.substack.com/p/the-untrainable
---

## TL;DR
- 이 글은 AI 기술의 발전이 기업 가치와 투자자 심리에 미치는 영향을 다룬다.
- 모델이 해결할 수 있는 문제는 증가하고 있으나, 사적이고 검증이 어려운 영역은 여전히 보호받고 있다는 점이 핵심 주장이다.
- 독자에게는 AI의 발전이 기업 전략과 투자 판단에 미치는 장기적 영향을 이해하는 데 중요한 통찰을 제공한다.

## GeekNews 요약
- 2026년 중반 투자자들 사이 "AI 정신증"적 절망론이 확산 중이며, 모델이 모든 것을 더 잘하게 되면 그 위에 세워진 회사는 모두 **흡수될 얇은 래퍼**에 불과하다는 논리가 핵심
- 첫 번째 AI SWE였던 Devin은 2024년 표준 벤치마크 과제의 13%만 해결했으나 1년 반 만에 최고 에이전트가 80%대 후반에 도달, **측정 가능한 영역**부터 모델이 빠르게 잠식 중
- 측정 가능한 것은 훈련 대상이 되어 commodity로 전락하지만, **사적이고 검증 비용이 큰 정답**은 리더보드로 읽어낼 수 없음
- 진정한 가치는 license·liability·책임 소재처럼 모델이 닿을 수 없는 영역에 있으며, 병목은 지능이 아니라 **권한(permission)과 책임(accountability)**
- 지능이 저렴해질수록 가치는 모델이 도달할 수 없는 소수의 자리로 이동, **‘훈련 불가능한 영역(untrainable)’** 이 최종 해자(moat)

---

## 원문
- [원문](https://saranormous.substack.com/p/the-untrainable)
- [GeekNews 토론](https://news.hada.io/topic?id=30712)

## My Note
<!-- 한 줄 코멘트 남기기 -->
