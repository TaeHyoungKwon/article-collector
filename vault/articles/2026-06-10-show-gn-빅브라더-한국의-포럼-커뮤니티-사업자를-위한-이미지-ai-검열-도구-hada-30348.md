---
category: AI
collected_at: '2026-06-10T11:35:25+09:00'
geeknews_comments: 0
geeknews_score: 2
geeknews_url: https://news.hada.io/topic?id=30348
id: hada-30348
matched_keywords:
- AI
read: false
recommend_score: 3.099
recommended_on: '2026-06-12'
source: geeknews
tags:
- AI
- Other
- github.com/NomaDamas
title: 'Show GN: 빅브라더 - 한국의 포럼/커뮤니티 사업자를 위한 이미지 AI 검열 도구'
url: https://github.com/NomaDamas/big-brother
---

## TL;DR
- 이 글은 한국의 포럼 및 커뮤니티 사업자를 위한 이미지 AI 검열 도구에 대한 내용을 다룬다.
- 새롭게 시행되는 법에 따라 사업자들은 AI 필터링을 구축해야 하며, 오픈소스 도구가 제공된다.
- 이는 커뮤니티 내에서 투명한 검열을 가능하게 하여 사용자 신뢰를 높일 수 있는 기회를 제공한다.

## GeekNews 요약
- 7월 1일부터 전기통신사업법 제22조의5제2항 및 같은 법 시행령 제30조의6에 따라서 국내 커뮤니티 및 포럼 사업자들은 사용자들의 모든 이미지들을 AI를 통해 확인하고 필터링해야 함.
- 웹사이트 운영자가 Nvidia GPU를 구매하고, 스스로 필터링을 위한 AI 서빙 및 소프트웨어를 구축해야 함.
- 한국의 커뮤니티 및 포럼 사업자들을 위하여 AI 검열 도구를 만들어 오픈소스로 공개함. MIT 라이선스.
- 유해물 감지 AI 모델은 NVIDIA GPU에 쉽게 서빙하여 이미지를 필터링.
- Hash bank도 포함되어 있어 이미 필터링 된 유해 이미지와 동일한 이미지는 GPU를 거치지 않고 필터링 가능.
- 이 오픈소스를 사용해 각종 커뮤니티에서 "투명한 검열"이 이루어지면 좋겠음.

---

빠르게 만들어본 프로젝트라 부족한 부분이 많습니다. 버그나 부족한 기능은 깃허브 이슈로 제보해주시면 개선하겠습니다.  
검열을 피할 수 없다면 그 검열의 수단 만이라도 투명했으면 좋겠습니다. 취지는 좋은 법이니깐요!!

많은 관심과 사용 부탁드립니다.

## 원문
- [원문](https://github.com/NomaDamas/big-brother)
- [GeekNews 토론](https://news.hada.io/topic?id=30348)

## My Note
<!-- 한 줄 코멘트 남기기 -->
